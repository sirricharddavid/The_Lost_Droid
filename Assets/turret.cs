...
public class turret : MonoBehaviour
{
    ...
    void shootWithCheck()
    {
        ...
        if (Physics.Raycast(shootPoint1.transform.position, shootPoint1.transform.TransformDirection(Vector3.forward), out hit))
        {
            if (hit.collider.gameObject.tag == "Player")
            {
                if (cooldownTimer <= 0)
                {
                    ...
                    if (turretShoot)
                    {
                        shootBullet = Instantiate(bullet, shootPoint1.transform.position, Cannon.transform.rotation);
                    ...
                    {
                        shootBullet = Instantiate(bullet, shootPoint2.transform.position, Cannon.transform.rotation);
                        shootBullet.GetComponent<Rigidbody>().velocity = Cannon.transform.TransformDirection(Vector3.forward * bulletSpeed);
                        shootBullet.GetComponent<bullet>().timer = bulletDuration;
                        shootBullet.GetComponent<bullet>().dmg = bulletDmg;
                        turretShoot = true;
